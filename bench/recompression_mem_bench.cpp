...
int main(int argc, char *argv[]) {
    if (argc < 6) {
        std::cerr << "./bench_recompression_mem [path] [file_name(s)] [sequential | parallel | parallel_lp | full_parallel | parallel_ls | parallel_gr | fast | hash | parallel_rnd] [cores] [repeats]" << std::endl;
        return -1;
    }
