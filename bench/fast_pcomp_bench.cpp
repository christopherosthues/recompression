...
//    std::cout << "RESULT dataset=" << dataset << " algo=parallel_bcomp";
    recompression.pcomp(text, rlslp, alpha, mapping);
    const auto endTime = std::chrono::system_clock::now();
    const auto timeSpan = endTime - startTime;
    LOG(INFO) << "Time for fast pcomp: " << std::chrono::duration_cast<std::chrono::seconds>(timeSpan).count() << "[s]";
    LOG(INFO) << "Time for fast pcomp: " << std::chrono::duration_cast<std::chrono::milliseconds>(timeSpan).count() << "[ms]";
