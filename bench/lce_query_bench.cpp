        ...
        for (size_t repeat = 0; repeat < repeats; ++repeat) {
            ...
            for (size_t l = 0; l < algos.size(); ++l) {
                ...
                if (algo == "recomp") {
                    ...
                    std::cout << "RESULT algo=recompression dataset=" << dataset << " queries=" << accesses << " lce="
                              << lces[l] << " time="
                              << std::chrono::duration_cast<std::chrono::milliseconds>(timeSpan).count() << std::endl;
                } else if (algo == "naive") {
                    const auto startTime = recomp::timer::now();
                    for (size_t i = 0; i < accesses; ++i) {
                        lces[l] += recomp::lce_query::lce_query_naive(plain_text, indices[i], indices[(i + 1) % accesses]);
                    }
                    const auto endTime = recomp::timer::now();
                    const auto timeSpan = endTime - startTime;
            ...
            for (size_t l = 0; l < algos.size() - 1; ++l) {
                if (lces[l] != lces[l + 1]) {
                    std::cout << "Failure: " << algos[l] << " " << lces[l] << " != " << lces[l + 1] << " " << algos[l + 1] << std::endl;
                }
            }
        }
    }
