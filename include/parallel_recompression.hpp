...
//                }
//                bounds[n_threads] = starts.size();
////                std::array<size_t, n_threads> t_starts;
...
//                index = starts[i];
//
//                while (symbol_index < alphabet.size() && std::get<0>(multiset[i]) > alphabet[symbol_index]) {
//
...
//                    r_count = 0;
//                }
//                omp_test_lock(locks[std::get<1>(multiset[i])]);
