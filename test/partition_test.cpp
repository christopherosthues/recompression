...
typedef parallel::recompression_fast<var_t, term_t>::adj_list_t par_fast_multiset_t;
typedef parallel::recompression_fast<var_t, term_t>::partition_t par_fast_partition_t;
...
TEST(parallel_partition, 212181623541741623541321) {
    ...
    parallel::compute_partition<par_multiset_t, par_partition_t>(multiset, partition);
...
TEST(parallel_fast_partition, 212181623541741623541321) {
    // TODO
    par_text_t text{2, 1, 2, 1, 8, 1, 6, 2, 3, 5, 4, 1, 7, 4, 1, 6, 2, 3, 5, 4, 1, 3, 2, 1};
    par_fast_multiset_t multiset(text.size() - 1);
    size_t begin = 0;
    bool part_l = false;
    par_alphabet_t alphabet{1, 2, 3, 4, 5, 6, 7, 8};
    par_fast_partition_t partition;
    for (const auto& a : alphabet) {
        partition[a] = false;
    }
    ...
    ips4o::parallel::sort(multiset.begin(), multiset.end());
    parallel::compute_partition<par_fast_multiset_t, par_fast_partition_t, var_t>(multiset, partition, begin, part_l);

    par_fast_partition_t exp_partition;
    exp_partition[1] = true;
    exp_partition[2] = false;
    exp_partition[3] = true;
    exp_partition[4] = false;
...
TEST(parallel_fast_partition, 131261051171161051139) {
    // TODO
    par_text_t text{13, 12, 6, 10, 5, 11, 7, 11, 6, 10, 5, 11, 3, 9};
    par_fast_multiset_t multiset(text.size() - 1);
    size_t begin = 0;
...
TEST(parallel_fast_partition, 18161517161514) {
    // TODO
    par_text_t text{18, 16, 15, 17, 16, 15, 14};
    par_fast_multiset_t multiset(text.size() - 1);
    size_t begin = 0;
...
TEST(parallel_fast_partition, 21201619) {
    // TODO
    par_text_t text{21, 20, 16, 19};
    par_fast_multiset_t multiset(text.size() - 1);
    size_t begin = 0;
...
TEST(parallel_fast_partition, 2322) {
    // TODO
    par_text_t text{23, 22};
    par_fast_multiset_t multiset(text.size() - 1);
    size_t begin = 0;
