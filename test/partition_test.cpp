...
typedef parallel::recompression_order<var_t, term_t>::adj_list_t parallel_order_adj_list_t;
typedef parallel::recompression_order<var_t, term_t>::partition_t par_order_partition_t;

TEST(parallel_order_partition, 212181623541741623541321) {
    // TODO
    par_text_t text{2, 1, 2, 1, 8, 1, 6, 2, 3, 5, 4, 1, 7, 4, 1, 6, 2, 3, 5, 4, 1, 3, 2, 1};
    parallel_order_adj_list_t multiset(text.size() - 1);
    size_t begin = 0;
    bool part_l = false;
    par_alphabet_t alphabet{1, 2, 3, 4, 5, 6, 7, 8};
    par_order_partition_t partition;
    for (const auto& a : alphabet) {
        partition[a] = false;
    }
    parallel::recompression_order<var_t, term_t> recomp;
    recomp.compute_adj_list(text, multiset, begin);
    ips4o::parallel::sort(multiset.begin(), multiset.end());
    parallel::compute_partition_order<parallel_order_adj_list_t, par_order_partition_t, var_t>(multiset, partition, begin, part_l);

    par_order_partition_t exp_partition;
...
TEST(parallel_order_partition, 131261051171161051139) {
    // TODO
    ...
    recomp.compute_adj_list(text, multiset, begin);
    ips4o::parallel::sort(multiset.begin(), multiset.end());
    parallel::compute_partition_order<parallel_order_adj_list_t, par_order_partition_t, var_t>(multiset, partition, begin,
                                                                                        part_l);
...
TEST(parallel_order_partition, 18161517161514) {
    // TODO
...
TEST(parallel_order_partition, 21201619) {
    // TODO
...
TEST(parallel_order_partition, 2322) {
    // TODO
